...
export const audioService = {
  ...
  getVoices: (): Promise<
    ApiResponse<{ id: string; name: string; category?: string }[]>
...
export const resourcesService = {
  ...
  quickSearch: (query: string): Promise<ApiResponse<ResourceLink[]>> =>
    api
      .get(`/linkup/search/${encodeURIComponent(query)}`)
      .then((res) => res.data),
};
...
export const bedrockService = {
  ...
  simplifyContent: (data: {
    text: string;
    targetLevel?: string;
  }): Promise<ApiResponse<{ simplifiedText: string; level: string }>> =>
  ...
  generatePEI: (data: {
    ...
    studentName: string;
    gradeLevel: string;
  }): Promise<ApiResponse<{ peiDraft: string; sections: string[] }>> =>
...
export interface AuthResponse {
  ...
  token?: string;
  user?: AuthUser;
  authUser?: AuthUser; // compatibilidad con backend alterno
  message?: string;
}

export const authService = {
  // Reducimos el timeout específicamente para login para evitar bloqueos largos en dev
  login: (email: string, password: string): Promise<AuthResponse> =>
    api
      .post("/auth/login", { email, password }, { timeout: 8000 })
