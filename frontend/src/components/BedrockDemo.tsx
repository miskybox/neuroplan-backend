...
// Helper para convertir unknown a string seguro
...
export const BedrockDemo: React.FC<BedrockDemoProps> = ({
  showTitle = true,
}) => {
  ...
  const [generatedPEI, setGeneratedPEI] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const handleSimplifyContent = async () => {
    ...
    try {
      ...
      setSimplifiedText(
        response.data.simplifiedText || toString(response.data)
      );
      toast.success("Texto simplificado exitosamente");
    } catch (error: unknown) {
      const errorMessage = toString(error);
      toast.error("Error al simplificar: " + errorMessage);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGeneratePEI = async () => {
    ...
    try {
      ...
      const errorMessage = toString(error);
      toast.error("Error al generar PEI: " + errorMessage);
    } finally {
      setIsLoading(false);
    }
  };

  // Función extraída para evitar ternarios anidados (SonarLint S3358)
      ...
      {showTitle && (
        <div className="flex items-center justify-between">
          ...
          <Badge variant={backendConnected ? "default" : "secondary"}>
            {backendConnected ? "Conectado" : "Demo"}
          </Badge>
        </div>
      )}
              ...
              {models.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {models.map((model, index) => (
                    <Card
                      key={`${toString(
                        model.name ?? model.modelId ?? index
                      )}-${index}`}
                      className="border-2"
                    >
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            <h4 className="font-semibold">
                              {toString(model.name || model.modelId)}
                            </h4>
                            <p className="text-sm text-muted-foreground">
                              {toString(model.provider || "AWS Bedrock")}
                            </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPeiData({ ...peiData, studentName: e.target.value })
                    }
                    className="w-full p-3 border rounded-lg"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPeiData({ ...peiData, gradeLevel: e.target.value })
                    }
                    className="w-full p-3 border rounded-lg"
                ...
                <Card className="bg-blue-50 border-blue-200">
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <pre className="bg-white p-4 rounded-lg overflow-x-auto max-h-[400px] font-mono text-xs">
                        {toString(generatedPEI)}
                      </pre>
                    </div>
                  </CardContent>
                </Card>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-800">
                  El backend no está disponible. Asegúrate de que esté
                  ejecutándose en{" "}
