...
export class StudentsController {
  ...
  async findAll(@CurrentUser() user: any) {
    try {
      ...
      const { data: students, error } = await this.databaseService.getStudentsByUser(userId);
  ...
  async create(@CurrentUser() user: any, @Body() studentDto: StudentDto) {
    try {
      ...
      const { data, error } = await this.databaseService.createStudent(newStudent);
  ...
  async update(@Param('id') id: string, @Body() studentDto: StudentDto, @CurrentUser() user: any) {
    const userId = user.id || user.userId;
    
    // Verificar que el estudiante pertenece al usuario
    const { data: existingStudent } = await this.databaseService.getClient()
      .from('students')
      .select('*')
      .eq('id', id)
      .eq('created_by', userId)
